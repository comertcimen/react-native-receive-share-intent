import type {
  IReceiveShareIntent,
  IUtils,
} from './ReceiveShareIntent.interface';
import { Platform, Linking, AppState, NativeModules } from 'react-native';
import { Utils } from './utils';

const { ReceiveShareIntent } = NativeModules;

export class ReceiveShareIntentModule implements IReceiveShareIntent {
  ...
  protected getFileNames(
    ...
    if (this.isIos) {
      ReceiveShareIntent.getFileNames(url)
        .then((data: any) => {
      ...
      ReceiveShareIntent.getFileNames()
        .then((fileObject: any) => {
