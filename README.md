...
```sh
npm install comertcimen/react-native-receive-share-intent
```

or

```sh
yarn add comertcimen/react-native-receive-share-intent
```

## Usage

```js
import ReceiveSharingIntent from 'react-native-receive-share-intent';
...
```
NOTE: On iOS, you'll need to add the LinkingIOS folder into your header search paths as described in step 3 https://reactnative.dev/docs/linking-libraries-ios#step-3. If you also want to listen to incoming app links during your app's execution, you'll need to add the following lines to your *AppDelegate.m
```

`<project_folder>/ios/<project_name>/AppDelegate.mm`
...
- Create a Share extension (start with host app bundle id "com.example.app.share")
...
- create a group for both the host app and the share extension ("group.com.example.app")
